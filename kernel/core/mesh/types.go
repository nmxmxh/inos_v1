...
// Types defined here align with protocols/schemas/p2p/v1/mesh.capnp
// and protocols/schemas/base/v1/base.capnp
// [/] Implement StorageProvider and StreamRPC interfaces in mesh/types.go
...
func (p *PeerCapability) ToCapnp(seg *capnp.Segment) (p2p.PeerCapability, error) {
	...
	// ConnectionState mapping is handled via cast if the enums match
	// If not, we should use a switch statement.
	// Protocols use p2p.ConnectionState.
	// cap.SetConnectionState(p2p.ConnectionState(p.ConnectionState))
...
func (p *PeerCapability) FromCapnp(cap p2p.PeerCapability) error {
	...
	// p.ConnectionState = ConnectionState(cap.ConnectionState())
...
func (e *Envelope) ToCapnp(seg *capnp.Segment) (base.Base_Envelope, error) {
	...
	if err == nil {
		...
		// typeId could be set based on e.Type or similar
...
func (m *ModelMetadata) Validate() error {
	...
	// Validate chunk mapping consistency
...
// This matches the structure in gossip.go
type GossipMessage struct {
	...
	Payload   interface{} `json:"payload"` // Can be any type - chunk announce, peer list, etc.
