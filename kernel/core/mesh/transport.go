...
type WebRTCTransport struct {
	nodeID          string
	localCapability *PeerCapability
...
func NewWebRTCTransport(nodeID string, config TransportConfig, logger *slog.Logger) (*WebRTCTransport, error) {
	...
	transport := &WebRTCTransport{
		...
		config:          config,
		logger:          logger.With("component", "transport", "node_id", nodeID[:8]),
		startTime:       time.Now(),
	}
...
func (t *WebRTCTransport) Connect(ctx context.Context, peerID string) error {
	...
	t.logger.Debug("connecting to peer", "peer", peerID[:8])

	// Try WebRTC first if enabled
	if t.config.WebRTCEnabled {
		if err := t.connectViaWebRTC(ctx, peerID); err == nil {
			t.logger.Debug("connected via WebRTC", "peer", peerID[:8])
			t.metricsMu.Lock()
			t.metrics.WebRTCCandidates++
			t.metricsMu.Unlock()
			return nil
		} else {
			t.logger.Debug("WebRTC connection failed", "peer", peerID[:8], "error", err)
		}
	}
	...
	t.logger.Debug("connected via WebSocket fallback", "peer", peerID[:8])
	t.metricsMu.Lock()
...
func (t *WebRTCTransport) connectViaWebRTC(ctx context.Context, peerID string) error {
	...
	peerConnection.OnICECandidate(func(candidate *webrtc.ICECandidate) {
		...
		if err != nil {
			t.logger.Error("failed to marshal ICE candidate", "error", err)
			return
		}
	...
	peerConnection.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		case webrtc.PeerConnectionStateConnected:
			...
			t.logger.Info("WebRTC connection established", "peer", peerID[:8])
	...
	case <-ctx.Done():
		peerConnection.Close()
		return ctx.Err()
	case <-time.After(t.config.RPCTimeout):
		peerConnection.Close()
		return errors.New("timeout waiting for WebRTC answer")
	}
}
...
func (t *WebRTCTransport) Disconnect(peerID string) error {
	...
	t.logger.Debug("disconnected from peer", "peer", peerID[:8])
	return nil
}
...
func (t *WebRTCTransport) GetPeerCapabilities(peerID string) (*PeerCapability, error) {
	// Check cache first
	t.connMu.RLock()
	conn, exists := t.connections[peerID]
	t.connMu.RUnlock()
...
func (t *WebRTCTransport) UpdateLocalCapabilities(capabilities *PeerCapability) error {
...
func (t *WebRTCTransport) FindPeers(ctx context.Context, key string) ([]PeerInfo, error) {
	// In production, this would query DHT and signaling server
	return nil, nil // Placeholder
}

// GetConnectionMetrics returns current transport metrics
func (t *WebRTCTransport) GetConnectionMetrics() ConnectionMetrics {
	t.metricsMu.RLock()
...
func (t *WebRTCTransport) GetHealth() TransportHealth {
...
func (t *WebRTCTransport) handleWebRTCOffer(senderID string, msg map[string]interface{}) {
	offerJSON, ok := msg["offer"].(string)
	if !ok {
		t.logger.Error("invalid WebRTC offer format")
		return
	}
	...
	if err != nil {
		peerConnection.Close()
		t.logger.Error("failed to marshal answer", "error", err)
		return
	}
...
func (t *WebRTCTransport) handleWebRTCAnswer(senderID string, msg map[string]interface{}) {
	answerJSON, ok := msg["answer"].(string)
	if !ok {
		t.logger.Error("invalid WebRTC answer format")
		return
	}
...
func (t *WebRTCTransport) handleICECandidate(senderID string, msg map[string]interface{}) {
	candidateJSON, ok := msg["candidate"].(string)
	if !ok {
		t.logger.Error("invalid ICE candidate format")
		return
	}
...
func (t *WebRTCTransport) FindNode(ctx context.Context, peerID, targetID string) ([]PeerInfo, error) {
	var nodes []PeerInfo
	err := t.SendRPC(ctx, peerID, "find_node", map[string]string{"target_id": targetID}, &nodes)
	return nodes, err
}

func (t *WebRTCTransport) FindValue(ctx context.Context, peerID, chunkHash string) ([]string, []PeerInfo, error) {
	var result struct {
