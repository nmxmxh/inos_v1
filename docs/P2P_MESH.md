...
INOS implements a **purely decentralized, adaptive P2P mesh** where every node acts as both a client and a server. There is **no centralization**—all signaling, resource discovery, and state synchronization happen over a resilient gossip mesh. 
...
### 3. Decentralized Signaling (Pure P2P)
We have removed the reliance on centralized signaling servers (previously `server.js`). Every node in the INOS mesh is now a signaling relay.
- **Gossip Relay**: WebRTC Session Descriptions (SDP) and ICE candidates are broadcasted over the Gossip mesh under the `webrtc.signaling` topic.
- **Bootstrap Nodes**: Nodes initially connect to a set of hardcoded bootstrap peers. Once inside the mesh, they discover all other peers through the DHT.
- **Resilience**: There is no single point of failure. If one node goes down, the signaling path automatically routes through other healthy peers.
...
**INOS P2P Mesh is:**
- ✅ **Decentralized Signaling** - No central server required for bootstrapping connections.
